...
struct SettingsHomeView: View {
    ...
    var body: some View {
        ...
        .alert(item: Binding<DeleteError?>(
                    get: { self.deleteError.map { DeleteError(message: $0) } },
                    set: { self.deleteError = $0?.message }
                )) { error in
    ...
    private func deleteUserData() {
        guard let userId = Auth.auth().currentUser?.uid else {
            ...
            self.deleteError = "No user is currently signed in"
            return
        }
        ...
        functions.httpsCallable("recursiveDelete").call { result, error in
            DispatchQueue.main.async {
                ...
                if let error = error as NSError? {
                    if error.domain == FunctionsErrorDomain {
                        ...
                        case .notFound:
                            print("User data not found. It may have already been deleted.")
                            self.handleSuccessfulDeletion()
                        default:
                            self.deleteError = "Error deleting user data: \(error.localizedDescription)"
                        }
                    } else {
                        self.deleteError = "Error deleting user data: \(error.localizedDescription)"
                    }
                    print(self.deleteError ?? "")
                } else if let resultData = result?.data as? [String: Any], let resultMessage = resultData["result"] as? String {
                    print("Cloud Function result: \(resultMessage)")
                    
                    if resultMessage == "Success" || resultMessage == "No data to delete" {
                        self.handleSuccessfulDeletion()
                    } else {
                        self.deleteError = "Unexpected result: \(resultMessage)"
                    }
                }
            }
        }
